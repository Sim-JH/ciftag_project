...
def run_tumblr(
    ...
    try:
        ...
        update_task_status(task_id, {
            'task_sta': enums.TaskStatusCode.failed.name,
            'msg': f'Max task retry over {PAGETYPE} (task_id: {task_id})',
        })
        raise CiftagWorkException(f'-- Max retry over {PAGETYPE} task_id: {task_id}', 400)
...
def after_tumblr(self, results: List[Dict[str, Any]], work_id: int, info_id: int, redis_name: str):
    from ciftag.models import enums
