...
class FlickrCrawlInfo(Base, CrawlInfoDataMixin):
    """플리커 크롤링 수행 정보"""

    __tablename__ = "flk_crawl_info"
...
class FlickrCrawlData(Base, ImgDataImgDataMixin):
    """플리커 크롤링 데이터"""

    __tablename__ = "flk_crawl_data"

    id = Column(Integer, primary_key=True)  # 인덱스
    flk_pk = Column(Integer)  # 핀터레스트 정보 ID (FK)
